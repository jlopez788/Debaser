...
IF NOT EXISTS (SELECT * FROM sys.types WHERE [name]='{DataTypeName}' AND [is_user_defined] = 1 AND [schema_id]={schemaId})
    CREATE TYPE [{_schema}].[{DataTypeName}] AS TABLE (
{GetColumnDefinitionSql(8)}
    )
...
IF NOT EXISTS (SELECT * FROM sys.objects WHERE [name]='{SprocName}' AND [type]='P' AND [schema_id]={schemaId})
    EXEC('CREATE PROCEDURE [{_schema}].[{SprocName}] AS BEGIN SET NOCOUNT ON; END')

");

						ExecuteStatement(connection, transaction, $@"

ALTER PROCEDURE [{_schema}].[{SprocName}] (
    @data [{_schema}].[{DataTypeName}] READONLY
)
AS
BEGIN
--     .___    ___.
			...
			using (var connection = new SqlConnection(_connectionString))
			{
				...
				if (dropProcedure)
				{
					using (var transaction = connection.BeginTransaction())
					{
						try
						{
							ExecuteStatement(connection, transaction, $@"DROP PROCEDURE [{_schema}].[{SprocName}]", wrapException: false);
				...
				if (dropType)
				{
					using (var transaction = connection.BeginTransaction())
					{
						try
						{
							ExecuteStatement(connection, transaction, $@"DROP TYPE [{_schema}].[{DataTypeName}]", wrapException: false);
