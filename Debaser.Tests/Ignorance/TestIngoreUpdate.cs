...
namespace Debaser.Tests.Ignorance
{
	...
	{
		private UpsertHelper<SomeRowWithIntegerRevision> _upsertHelper;
		private UpsertHelper<SomeRowWithDateTimeRevision> _upsertHelper2;
		...
		private class SomeRowWithIntegerRevision
		{
		...
		public async Task WorksWithDateTimeCriteriaToo()
		{
			...
			await _upsertHelper2.Modify(new[] { new SomeRowWithDateTimeRevision(1, "hej", t0) });

			Assert.That(_upsertHelper2.LoadAll().Single().Data, Is.EqualTo("hej"));

			await _upsertHelper2.Modify(new[] { new SomeRowWithDateTimeRevision(1, "hej igen", t1) });

			Assert.That(_upsertHelper2.LoadAll().Single().Data, Is.EqualTo("hej igen"));

			await _upsertHelper2.Modify(new[] { new SomeRowWithDateTimeRevision(1, "DEN HER SKAL IKKE IGENNEM", tminus1) });
		...
		private class SomeRowWithDateTimeRevision
		{
